...
struct Prediction {
    ...
    static func predictionArrayString(predictions: [Prediction]) -> ([String], [String]) {
        var c: [String] = []
        var p: [String] = []
        
        for (i, prediction) in predictions.enumerated() {
            c.append(prediction.category)
            p.append(String(format: "(%3.2f%%)", prediction.probability * 100))
        }
        return (c, p)
    }
}
